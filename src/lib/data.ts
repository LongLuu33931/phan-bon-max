...
function mapProduct(row: Record<string, unknown>): Product {
    ...
    return {
        ...
        benefits: Array.isArray(row.benefits)
            ? (row.benefits as Product["benefits"])
            : [],
        usageRows: Array.isArray(row.usage_rows)
            ? (row.usage_rows as Product["usageRows"])
            : [],
...
function mapPost(row: Record<string, unknown>): Post {
    return {
        ...
        postType: row.post_type === "press" ? "press" : "article",
...
export async function getAllProductsForAdmin() {
    if (!supabaseConfigured && postgresConfigured) {
        ...
        const pool = getPostgresPool();
        const { rows } = await pool!.query(
            "select * from public.products order by updated_at desc",
        );
        return rows.map(mapProduct);
    }
...
export async function getAllPostsForAdmin() {
    if (!supabaseConfigured && postgresConfigured) {
        ...
        const pool = getPostgresPool();
        const { rows } = await pool!.query(
            "select * from public.posts order by updated_at desc",
        );
        return rows.map(mapPost);
    }
...
export async function getTestimonials() {
    ...
    if (!supabaseConfigured)
        return testimonials.filter((item) => item.isActive);
    noStore();
    const supabase = await createSupabaseServerClient();
    const { data, error } = await supabase!
        .from("testimonials")
    ...
    return error || !data?.length
        ? testimonials.filter((item) => item.isActive)
...
export async function getAllTestimonialsForAdmin() {
    if (!supabaseConfigured && postgresConfigured) {
        ...
        const pool = getPostgresPool();
        const { rows } = await pool!.query(
            "select * from public.testimonials order by sort_order asc, updated_at desc",
        );
        return rows.map(mapTestimonial);
    }
...
export async function getAdminNotificationCounts() {
    if (!supabaseConfigured && postgresConfigured) {
        ...
        const [orders, contacts] = await Promise.all([
            ...
            pool!.query(
                "select count(*)::int as count from public.contact_messages where status = 'new'",
            ),
        ]);
