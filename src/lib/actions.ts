...
async function ensurePostImagesBucket() {
    ...
    await adminSupabase.storage.createBucket(POST_IMAGE_BUCKET, {
        public: true,
    });
}
...
async function ensureSiteAssetsBucket() {
    ...
    await adminSupabase.storage.createBucket(SITE_ASSETS_BUCKET, {
        public: true,
    });
}
...
export async function updateOrderStatus(
    ...
    if (!supabaseConfigured && !postgresConfigured) {
        return {
            ok: false,
            message: "Chưa cấu hình database để cập nhật đơn hàng.",
        };
    }
    ...
    if (supabaseConfigured) {
        ...
        await pool!.query(
            "update public.orders set status = $1, updated_at = now() where id = $2::uuid",
...
export async function updateOrderCustomerInfo(
    ...
    const parsed = orderCustomerInfoSchema.safeParse(
        Object.fromEntries(formData.entries()),
    );
    if (!parsed.success) {
        return {
            ...
            message:
                ...
                "Thông tin giao hàng chưa hợp lệ.",
        };
    }
    ...
    if (supabaseConfigured) {
        ...
        const pool = getPostgresPool();
        await pool!.query(
            "update public.orders set address = $1, province = $2, note = $3, updated_at = now() where id = $4::uuid",
            [payload.address, payload.province, payload.note, parsed.data.id],
        );
    }
...
export async function updateContactMessageStatus(
    ...
    if (!supabaseConfigured && !postgresConfigured) {
        return {
            ok: false,
            message: "Chưa cấu hình database để cập nhật liên hệ.",
        };
    }
    ...
    if (supabaseConfigured) {
        ...
        const pool = getPostgresPool();
        await pool!.query(
            "update public.contact_messages set status = $1, updated_at = now() where id = $2::uuid",
            [parsed.data.status, parsed.data.id],
        );
    }
...
export async function saveContactMessage(
    ...
    if (!supabaseConfigured && !postgresConfigured) {
        return {
            ok: false,
            message: "Chưa cấu hình Supabase env để nhận liên hệ.",
        };
    }
    ...
    return {
        ok: true,
        message: "Đã gửi thông tin. MAX 8000 sẽ liên hệ lại sớm.",
    };
}

export async function createCheckoutOrder(
    payload: unknown,
): Promise<ActionState & { orderCode?: string }> {
    ...
    let sourceProducts: Array<{ id: string; name: string; price: number }> =
        ...
        try {
            ...
            await resend.emails.send({
                from:
                    ...
                    "MAX 8000 <orders@phanbonmax8000.com>",
            ...
            return {
                ok: true,
                orderCode,
                message: "Đơn đã lưu, email thông báo chưa gửi được.",
            };
        }
    }
...
export async function saveTestimonial(
    ...
    if (!parsed.success) {
        return {
            ok: false,
            message: parsed.error.issues[0]?.message ?? "Feedback chưa hợp lệ",
        };
    }
    ...
    if (!supabaseConfigured && !postgresConfigured) {
        return {
            ok: false,
            message:
                "Chưa cấu hình Supabase env. Form đã sẵn sàng, hãy thêm env rồi lưu vào database thật.",
        };
    }
        ...
        try {
            ...
            return {
                ...
                message:
                    error instanceof Error
                        ? error.message
                        : "Không upload được ảnh feedback.",
            };
        }
    }
...
export async function savePost(
    ...
    if (!parsed.success) {
        return {
            ok: false,
            message: parsed.error.issues[0]?.message ?? "Bài viết chưa hợp lệ",
        };
    }
    ...
    if (!supabaseConfigured && !postgresConfigured) {
        return {
            ok: false,
            message:
                "Chưa cấu hình database. Form đã sẵn sàng, hãy thêm env rồi lưu vào database thật.",
        };
    }

    const requestedPublishedAt = vietnamDateTimeLocalToIso(
    ...
    try {
        if (coverImage instanceof File && coverImage.size > 0) {
            coverImageUrl = await uploadPostImageFile(
                coverImage,
                parsed.data.slug,
            );
        }
    ...
    const payload = {
        ...
        updated_at: new Date().toISOString(),
        post_type: parsed.data.postType || "article",
    ...
    if (supabaseConfigured) {
        ...
        if (id) {
          ...
          seo_description=$7,status=$8,published_at=$9,post_type=$10,external_url=$11,updated_at=now()
         where id=$12::uuid`,
                values,
            );
        } else {
          ...
          (title,slug,excerpt,content,cover_image_url,seo_title,seo_description,status,published_at,post_type,external_url)
...
export async function togglePostPublished(
    ...
    if (!supabaseConfigured && !postgresConfigured) {
        return {
            ok: false,
            message: "Chưa cấu hình database để cập nhật bài viết.",
        };
    }
    ...
    return {
        ok: true,
        message: isPublished ? "Đã hiển thị bài viết." : "Đã ẩn bài viết.",
    };
}
...
export async function saveSiteSettings(
    ...
    try {
        ...
        return {
            ok: false,
            message: "Dữ liệu hero slider chưa đúng định dạng.",
        };
    }
    ...
    if (supabaseConfigured) {
        ...
        if (logo instanceof File && logo.size > 0) {
            try {
                const oldLogoPath = logoUrl
                    ? getStoragePublicPath(logoUrl, SITE_ASSETS_BUCKET)
                    : null;
        ...
        if (!logoUrl)
            return { ok: false, message: "Vui lòng upload logo website." };
        ...
        try {
            for (let index = 0; index < heroSlides.length; index++) {
                ...
                if (image instanceof File && image.size > 0) {
                    heroSlides[index] = {
                        ...heroSlides[index],
                        imageUrl: await uploadSiteAssetFile(
                            image,
                            "hero-slides",
                        ),
                    };
...
export async function saveProduct(
    ...
    const originalThumbnailUrl =
        formData.get("originalThumbnailUrl")?.toString() || undefined;
    let thumbnailUrl = formData.get("thumbnailUrl")?.toString() || undefined;
    const newGalleryThumbnailIndexValue = formData
        .get("newGalleryThumbnailIndex")
        ?.toString();
    const newGalleryThumbnailIndex =
        newGalleryThumbnailIndexValue === undefined ||
        newGalleryThumbnailIndexValue === ""
            ? null
    ...
    try {
        ...
        const originalImagesValue = formData
            .get("originalImagesJson")
            ?.toString();
        if (originalImagesValue)
            originalImages = JSON.parse(originalImagesValue);
    } catch {
        return {
            ok: false,
            message: "Dữ liệu hình ảnh hoặc nội dung chưa đúng định dạng.",
        };
    }
        ...
        for (let index = 0; index < images.length; index++) {
            ...
            const { error: uploadError } = await supabase!.storage
                ...
                .upload(path, replacement, {
                    upsert: false,
                    contentType: replacement.type || undefined,
                });
            ...
            images[index] = {
                ...images[index],
                url: data.publicUrl,
                alt: images[index]?.alt || parsed.data.name,
            };
        }
    }
    ...
    const selectedNewGalleryIndex =
        typeof newGalleryThumbnailIndex === "number" &&
    ...
    if (supabaseConfigured && selectedNewGalleryIndex !== null) {
        images = images.map((image, index) => ({
            ...image,
            isPrimary:
                index === firstNewGalleryImageIndex + selectedNewGalleryIndex,
        }));
    }
    ...
    images = images.map((image) => ({
        ...image,
        isPrimary: Boolean(thumbnailUrl && image.url === thumbnailUrl),
    }));
    ...
    if (supabaseConfigured) {
        ...
        if (
            ...
            !keptImageUrls.has(originalThumbnailUrl)
        ) {
            const thumbnailPath =
                getProductImageStoragePath(originalThumbnailUrl);
...
export async function toggleProductActive(
    ...
    if (!supabaseConfigured && !postgresConfigured) {
        return {
            ok: false,
            message: "Chưa cấu hình database để cập nhật sản phẩm.",
        };
    }
    ...
    if (supabaseConfigured) {
        ...
        const pool = getPostgresPool();
        await pool!.query(
            "update public.products set is_active = $1, updated_at = now() where id = $2::uuid",
            [isActive, id],
        );
    }
...
export async function saveCategory(
    ...
    if (!supabaseConfigured && !postgresConfigured) {
        return {
            ok: false,
            message: "Chưa cấu hình database để lưu danh mục.",
        };
    }
...
export async function toggleCategoryActive(
    ...
    if (!supabaseConfigured && !postgresConfigured) {
        return {
            ok: false,
            message: "Chưa cấu hình database để cập nhật danh mục.",
        };
    }
    ...
    if (supabaseConfigured) {
        ...
        const pool = getPostgresPool();
        await pool!.query(
            "update public.categories set is_active = $1, updated_at = now() where id = $2::uuid",
            [isActive, id],
        );
    }
    ...
    return {
        ok: true,
        message: isActive ? "Đã hiển thị danh mục." : "Đã ẩn danh mục.",
    };
}
...
export async function deleteCategory(id: string): Promise<ActionState> {
    ...
    if (!supabaseConfigured && !postgresConfigured) {
        return {
            ok: false,
            message: "Chưa cấu hình database để xóa danh mục.",
        };
    }
    ...
    if (productCount > 0) {
        return {
            ok: false,
            message: `Không thể xóa vì đang có ${productCount} sản phẩm dùng danh mục này.`,
        };
    }
