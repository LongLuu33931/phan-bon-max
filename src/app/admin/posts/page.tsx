  ...
  return (
    <AdminShell>
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <div>
          <p className="text-sm font-bold uppercase tracking-[0.14em] text-emerald-700">Nội dung</p>
          <h1 className="mt-2 text-3xl font-black tracking-tight text-stone-950">Tin tức</h1>
        </div>
      ...
      <div className="mt-7 overflow-hidden rounded-2xl border border-stone-200 bg-white shadow-[0_10px_30px_rgba(15,23,42,0.04)]">
        {posts.map((post) => (
          <Link key={post.id} href={`/admin/posts/${post.id}`} className="grid gap-3 border-b border-stone-100 p-5 transition hover:bg-stone-50 md:grid-cols-[1fr_auto] md:items-center">
            <div>
