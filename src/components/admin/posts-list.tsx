...
export function PostsList({ posts }: { posts: Post[] }) {
  ...
  function handleToggle(post: Post) {
    ...
    const nextPublishedAt = nextPublished ? (post.publishedAt || new Date().toISOString()) : "";

    setPendingId(post.id);
    setRows((items) => items.map((item) => (
      item.id === post.id ? { ...item, status: nextStatus, publishedAt: nextPublishedAt } : item
    )));
    ...
    startTransition(async () => {
      ...
      setRows((items) => items.map((item) => (item.id === post.id ? post : item)));
    });
  }
        ...
        <table className="w-full min-w-[980px] text-left text-sm">
          ...
          <tbody className="divide-y divide-stone-100">
            {rows.map((post) => {
              ...
              return (
                <tr key={post.id} className="align-middle transition hover:bg-stone-50">
                  <td className="max-w-3xl px-5 py-4">
                    <p className="line-clamp-1 font-black text-stone-950">{post.title}</p>
                    <p className="mt-1 line-clamp-2 text-sm leading-6 text-stone-600">{post.excerpt}</p>
                  </td>
                  <td className="px-5 py-4 font-bold text-amber-700">
                    {post.publishedAt ? formatDate(post.publishedAt) : "Chưa đặt"}
                  </td>
                  <td className="px-5 py-4">
                    <span className={isPublished ? "rounded-full bg-emerald-50 px-3 py-1 text-xs font-black text-emerald-700" : "rounded-full bg-stone-100 px-3 py-1 text-xs font-black text-stone-500"}>
                  ...
                  <td className="px-5 py-4">
                    ...
                    >
                      ...
                      />
                      <span className="sr-only">{isPublished ? "Ẩn bài viết" : "Hiển thị bài viết"}</span>
                    </button>
                  </td>
                  <td className="px-5 py-4 text-right">
                    <Link href={`/admin/posts/${post.id}`} className="inline-flex h-9 items-center gap-2 rounded-lg border border-stone-200 px-3 font-bold text-stone-700 hover:border-emerald-200 hover:text-emerald-800">
                      <Edit3 size={15} /> Sửa
                    </Link>
                  </td>
                </tr>
