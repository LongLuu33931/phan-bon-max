...
const inputClass =
    "h-12 rounded-xl border border-stone-200 bg-white px-4 font-normal text-stone-900 outline-none transition focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100";
const textareaClass =
    "h-full w-full resize-none rounded-none border-0 bg-white px-4 py-3 font-mono text-sm font-normal leading-7 text-stone-900 outline-none";
...
export function PostForm({ post }: { post?: Post }) {
    ...
    const [postType, setPostType] = useState<"article" | "press">(
    ...
    function insertMarkdown(
        ...
        const formatted = mode
            ? formatSelectedLines(selected, before, mode)
    ...
    async function uploadInlineImage(file: File) {
        ...
        try {
            ...
            const alt =
                file.name.replace(/\.[^.]+$/, "") || "Hình ảnh bài viết";
    ...
    return (
        <form action={action} className="grid gap-5 pb-28">
            ...
            <section className="rounded-2xl border border-stone-200 bg-white p-6 shadow-[0_10px_30px_rgba(15,23,42,0.04)]">
                <div className="border-b border-stone-100 pb-5">
                    ...
                    <p className="mt-1 text-sm text-stone-500">
                        Tiêu đề, slug tự động, ảnh bìa và mô tả ngắn dùng cho
                        danh sách tin tức.
                    </p>
                </div>
                <div className="mt-5 grid gap-5 lg:grid-cols-2">
                    <label className={`${labelClass} lg:col-span-2`}>
                        ...
                        <select
                            ...
                            onChange={(event) =>
                                setPostType(
                                    event.target.value as "article" | "press",
                                )
                            }
                    ...
                    <div className="grid gap-2 text-sm font-bold text-stone-700">
                        ...
                        <div className="flex h-12 min-w-0 items-center overflow-hidden rounded-xl border border-stone-200 bg-stone-50 px-4 font-normal text-stone-500">
                            <span className="truncate">
                                {resolvedSlug || "slug-se-tu-tao-theo-tieu-de"}
                            </span>
                        </div>
                    </div>
                    ...
                    {postType === "press" ? (
                        <label className={`${labelClass} lg:col-span-2`}>
                            Link báo chí (URL bên ngoài)
                            <input
                                value={externalUrl}
                                onChange={(event) =>
                                    setExternalUrl(event.target.value)
                                }
                                placeholder="https://baomoi.com/bai-viet-ve-san-pham..."
                                className={inputClass}
                            />
                        </label>
                    ) : null}
                    <label className={`${labelClass} lg:col-span-2`}>
                        ...
                        <textarea
                            ...
                            required
                            className="min-h-[120px] rounded-xl border border-stone-200 bg-white px-4 py-3 text-base font-normal text-stone-900 outline-none transition focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100"
                        />
                    </label>
                    {coverPreview ? (
            ...
            {postType === "article" ? (
                <section className="rounded-2xl border border-stone-200 bg-white p-6 shadow-[0_10px_30px_rgba(15,23,42,0.04)]">
                    <div className="border-b border-stone-100 pb-5">
                        ...
                        <p className="mt-1 text-sm text-stone-500">
                            Nút hình ảnh sẽ upload lên Supabase rồi chèn vào bài
                            viết.
                        </p>
                    </div>
                    {uploadMessage ? (
                        <p
                            className={
                                uploadMessage.startsWith("Đã")
                                    ? "mt-3 text-sm font-bold text-emerald-700"
                                    : "mt-3 text-sm font-bold text-amber-700"
                            }
                        >
                            {uploadMessage}
                        </p>
            ...
            <AdminFormActions message={state.message} ok={state.ok}>
                <button
                    disabled={isPending}
                    className="inline-flex h-12 items-center gap-2 rounded-xl bg-emerald-900 px-5 text-sm font-black text-white shadow-sm transition hover:bg-emerald-800 disabled:bg-stone-300"
                >
                    {isPending ? <UploadCloud size={17} /> : <Save size={17} />}{" "}
